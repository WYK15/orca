...
`linear-tickets` is the legacy bundled name for `orca-linear`. This copy remains complete; its CLI commands are identical to `orca-linear` and always use `orcaw linear ...`.

Use `orcaw linear` when Linear is the source of task context or ticket updates. On Linux, use `orcaw-ide` wherever this file says `orcaw`.

`orca-linear` and `linear-tickets` are skill names, not CLI namespaces. Always run `orcaw linear ...` commands.
...
If the installed CLI help disagrees with this skill, trust `orcaw linear --help` for the available command surface and tell the user the skill guidance may be stale.
...
```bash
orcaw linear issue --current --full --json
```

Use search when the task names a ticket but the current worktree is not linked:

```bash
orcaw linear search "auth bug" --workspace all --limit 10 --json
orcaw linear issue ENG-123 --full --json
```
...
Do not use `orcaw linear attach` to read screenshots. That command creates link attachments, such as PR/MR links, and does not retrieve inline media files.

## Common Commands

```bash
orcaw linear save-issue [<id>] [--current] [--team <key|id>] [--title <title>] [--description <text> | --body-file <path|->] [--state <state>] [--assignee me|<user>|null] [--priority none|low|medium|high|urgent] [--estimate <number>|null] [--due-date <yyyy-mm-dd>|null] [--label <label>]... [--project <project>|null] [--parent-id <issue>|null] [--write-id <uuid>] [--workspace <id>] [--json]
orcaw linear issue [<id>] [--current] [--comments] [--children] [--depth <n>] [--attachments] [--relations] [--activity] [--full] [--workspace <id>] [--json]
orcaw linear list-issues [--team <team>] [--cycle <cycle>] [--label <label>] [--limit <n>] [--query <text>] [--state <state>] [--cursor <cursor>] [--order-by createdAt|updatedAt] [--project <project>] [--release <release>] [--assignee <user|me|null>] [--delegate <user|me|null>] [--parent-id <issue|null>] [--priority <0-4>] [--created-at <datetime|duration>] [--updated-at <datetime|duration>] [--include-archived] [--workspace <id>|all] [--json]
orcaw linear relation add [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
orcaw linear relation remove [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
orcaw linear search <query> [--limit <n>] [--workspace <id>|all] [--json]
orcaw linear team list [--workspace <id>|all] [--json]
orcaw linear team members --team <key|id> [--workspace <id>] [--json]
orcaw linear team states --team <key|id> [--workspace <id>] [--json]
orcaw linear team labels --team <key|id> [--workspace <id>] [--json]
orcaw linear project list [--query <text>] [--limit <n>] [--workspace <id>|all] [--json]
orcaw linear list [--filter assigned|created|all|completed|open] [--team <key|id>] [--limit <n>] [--workspace <id>|all] [--json]
orcaw linear status set [<id>] [--current] --to <state> [--workspace <id>] [--json]
orcaw linear assignee set [<id>] [--current] (--me | --to-id <userId>) [--workspace <id>] [--json]
orcaw linear assignee clear [<id>] [--current] [--workspace <id>] [--json]
orcaw linear priority set [<id>] [--current] --to none|low|medium|high|urgent [--workspace <id>] [--json]
orcaw linear priority clear [<id>] [--current] [--workspace <id>] [--json]
orcaw linear estimate set [<id>] [--current] --to <number> [--workspace <id>] [--json]
orcaw linear estimate clear [<id>] [--current] [--workspace <id>] [--json]
orcaw linear due-date set [<id>] [--current] --to <yyyy-mm-dd> [--workspace <id>] [--json]
orcaw linear due-date clear [<id>] [--current] [--workspace <id>] [--json]
orcaw linear label add [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
orcaw linear label remove [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
orcaw linear label set [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
orcaw linear comment add [<id>] [--current] (--body <text> | --body-file <path|->) [--reply-to <commentId>] [--write-id <uuid>] [--workspace <id>] [--json]
orcaw linear attach [<id>] [--current] --url <url> [--title <title>] [--write-id <uuid>] [--workspace <id>] [--json]
orcaw linear create --title <title> [--body <text> | --body-file <path|->] [--team <key|id>] [--project <projectId-or-exact-name>] [--state <stateId|exact-name>] [--assignee me|<userId>] [--priority none|low|medium|high|urgent] [--estimate <number>] [--due-date <yyyy-mm-dd>] [--label <labelId-or-exact-name>]... [--parent <id> | --parent-current] [--write-id <uuid>] [--workspace <id>] [--json]
```
...
```bash
orcaw linear team list --workspace all --json
orcaw linear team states --team <key-or-id> --workspace <workspaceId> --json
orcaw linear team labels --team <key-or-id> --workspace <workspaceId> --json
orcaw linear team members --team <key-or-id> --workspace <workspaceId> --json
orcaw linear project list --query <project-name> --workspace <workspaceId> --json
```
...
```bash
orcaw linear list --filter assigned --limit 10 --workspace all --json
orcaw linear list --filter open --team <key-or-id> --workspace <workspaceId> --json
```
...
The PR/MR command is `orcaw linear attach`; there is no `attach-pr` command.

Attach the PR/MR link:

```bash
orcaw linear attach --current --url <pr-or-mr-url> --title "PR/MR link" --json
```

Use stdin for multiline comments:

```bash
orcaw linear comment add --current --body-file - --json
```
...
2. Otherwise try `orcaw linear status set --current --to "In Review" --json`.
...
```bash
orcaw linear create --title <title> --parent-current --body-file - --json
```
...
```bash
orcaw linear issue <id> --workspace <workspaceId> --json
```
...
Confirm `orcaw status --json` unless already checked this turn, then read the current issue with `orcaw linear issue --current --full --json`. For completion, attach the PR/MR link, add one completion comment, and move status only when the target state is deterministic and non-regressive.
