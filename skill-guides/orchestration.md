...
If a task says to use Orca orchestration, the coordinator must create or bind a Run, create the Task with `orcaw orchestration task-create`, then attach the worker with either the preferred `orcaw orchestration worker-start` composition or the low-level `orcaw orchestration dispatch --inject` path.
...
```bash
orcaw orchestration task-list --json
orcaw orchestration dispatch-show --task <task_id> --json
```
...
- `orcaw status --json` should show a running runtime.
- `orcaw` must be on PATH (`orcaw-ide` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `orcaw orchestration` commands are RPC calls to the running Orca runtime.
...
```bash
orcaw orchestration run-use --id <adopted_run_id> --takeover-legacy --json
orcaw orchestration check --run <adopted_run_id> --json
```
...
- Do not use `orcaw orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `orcaw orchestration task-create`, `orcaw orchestration dispatch --inject`, or `orcaw orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
orcaw orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
orcaw orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
orcaw orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
orcaw orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
orcaw orchestration inbox [--limit <n>] [--json]
```
...
- Terminal handles remain appropriate for low-level pre-Dispatch messaging. Prefer `agentTerminalHandle` from the create response, fall back to `startupTerminal.handle` for older runtimes, then re-resolve with `orcaw terminal list --worktree ... --json` if missing or stale. Continue with the replacement handle only; never dual-send to old and new handles.
- `orcaw orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `orcaw orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
```bash
orcaw orchestration run-create --objective <text> --json
orcaw orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
orcaw orchestration task-list [--status <status>] [--ready] [--brief] [--json]
orcaw orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
orcaw orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
orcaw orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `orcaw terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
orcaw orchestration run-create --objective "<objective>" --json
orcaw orchestration task-create --spec "<worker A task>" --json
orcaw orchestration task-create --spec "<worker B task>" --json
orcaw orchestration worker-start --task <task_a> --worktree current --agent codex --json
orcaw orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
orcaw orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
orcaw orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
orcaw orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
orcaw orchestration worker-show --dispatch <dispatch_id> --json
orcaw orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
orcaw orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
```bash
orcaw orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
# Process every message in the returned Delivery, then atomically ack and continue:
orcaw orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
...
```bash
orcaw orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
orcaw orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
orcaw orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
orcaw orchestration reply --id <message_id> --body "<answer>" --json
```
...
```bash
orcaw orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
orcaw orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
orcaw orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `orcaw orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
```bash
orcaw terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Custom Codex model/effort handoff:

`orcaw worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `orcaw worktree create --json`; a bare repo id cannot target the new worktree.

```bash
orcaw worktree create --name <task-name> --no-parent --setup run --json
orcaw terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
orcaw terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
orcaw terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Wait only for `tui-idle` when needed to avoid losing the prompt. Do not monitor task completion.

`--no-parent` only controls Orca lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Orca uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `orcaw repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
orcaw terminal create --worktree active --title <task-name> --command "codex" --json
orcaw terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
orcaw orchestration dispatch --task <task_id> --to <handle> --inject --json
```
...
```bash
orcaw worktree create --name <task-name> --agent codex --setup run --json
...
Use `orcaw worktree create --prompt ...` or `orcaw terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
orcaw terminal list [--worktree <selector>] [--json]
orcaw terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
orcaw terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
orcaw terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
orcaw terminal read --terminal <handle> --json
orcaw terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `orcaw terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `orcaw orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
...
- After sending `worker_done`, end your turn and idle at the agent prompt. Do not poll or keep calling `orcaw orchestration check`; the coordinator re-engages you with a fresh preamble + TASK block delivered as new terminal input.
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `orcaw orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
orcaw terminal create --worktree active --title login-css-worker --command "claude" --json
orcaw terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
orcaw orchestration task-create --spec "Fix the login button CSS" --json
orcaw orchestration dispatch --task <task_id> --to <handle> --inject --json
orcaw orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
