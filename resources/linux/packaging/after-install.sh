#!/bin/bash
# Why: register the bundled `orcaw-ide` CLI on PATH at package-install time.
# The in-app "Install CLI" action (CliInstaller) can never run on a headless
# server, so without this symlink `orcaw-ide serve` is unreachable from the shell on
...
# symlink works. We discover the install dir instead of hardcoding one layout
# because electron-builder's directory name can vary by productName sanitization.
set -e

link="/usr/bin/orcaw-ide"

for dir in /opt/Orcaw /opt/orcaw-ide; do
  ...
  shim="$dir/resources/bin/orcaw-ide"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/orcaw-ide.
