...
const devChannelBuildVersion = isMacHourly
  ...
  : isMacAdhoc
    ...
    : undefined
// Why: retain upstream channel version semantics while every downstream package
// stays bound to the fork's application and release identity.
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing ${productIdentity.computerUseAppName} helper app at ${helperAppPath}`)
    }
    return
  }
...
async function signMacNotificationStatusHelper(helperPath, packager) {
  ...
  // before the outer Orcaw.app is sealed, like the computer-use helper.
