...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `orcaw-ide`;
  // other execution hosts use the native `orcaw` command.
...
export function buildDispatchPreamble(params: PreambleParams): string {
  ...
  const cli = params.devMode ? 'orca-dev' : (params.cliCommand ?? 'orcaw')
