...
describe('readPackagedReleaseAutoUpdateEnabled', () => {
  ...
  it('keeps Linux packages automatic', () => {
...
describe('createAvailableReleaseStatus', () => {
  it('includes manual delivery and the matching fork release URL', () => {
    expect(createAvailableReleaseStatus('1.4.187-wyk.2', null, 'manual')).toEqual({
      releaseUrl: 'https://github.com/WYK15/orca/releases/tag/v1.4.187-wyk.2',
      status: {
        state: 'available',
        version: '1.4.187-wyk.2',
        changelog: null,
        delivery: 'manual',
        releaseUrl: 'https://github.com/WYK15/orca/releases/tag/v1.4.187-wyk.2'
      }
  ...
  it('keeps automatic status payloads backward-compatible', () => {
    expect(createAvailableReleaseStatus('1.4.187-wyk.2', null, 'automatic').status).toEqual({
      state: 'available',
      version: '1.4.187-wyk.2',
      changelog: null
    })
