...
export const AUTOMATION_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage: 'orcaw automations list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['orcaw automations list', 'orcaw automations list --json']
  },
  {
    path: ['automations', 'show'],
    summary: 'Show one Orca automation',
    usage: 'orcaw automations show <id> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'id'],
    positionalArgs: ['id'],
    examples: ['orcaw automations show 2f9e...', 'orcaw automations show --id 2f9e... --json']
  },
  {
    path: ['automations', 'create'],
    summary: 'Create a scheduled Orca automation',
    usage:
      'orcaw automations create --name <name> --trigger <preset|cron|rrule> --prompt <text> --provider <agent> [--precheck <command>] [--repo <selector>|--workspace <selector>|--project <id> [--host <id>]|--project-host-setup <id>] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'name',
      'prompt',
    ...
    examples: [
      'orcaw automations create --name "Daily review" --trigger daily --prompt "Review open changes" --provider codex',
      'orcaw automations create --name "Weekday triage" --trigger "0 9 * * 1-5" --prompt "Triage issues" --provider claude --repo my-repo',
      'orcaw automations create --name "PR review" --trigger hourly --precheck "gh pr list --json number -q .[0].number" --prompt "Review requested PRs" --provider codex'
    ]
  },
  {
    path: ['automations', 'edit'],
    summary: 'Edit an Orca automation',
    usage: 'orcaw automations edit <id> [--name <name>] [--trigger <preset|cron|rrule>] [--json]',
    allowedFlags: [
      ...GLOBAL_FLAGS,
      'id',
      'name',
    ...
    positionalArgs: ['id'],
    examples: [
      'orcaw automations edit 2f9e... --disabled',
      'orcaw automations edit --id 2f9e... --trigger "30 * * * *" --json'
    ]
  },
  {
    ...
    usage: 'orcaw automations remove <id> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'id'],
    positionalArgs: ['id'],
    examples: ['orcaw automations remove 2f9e...', 'orcaw automations remove --id 2f9e... --json']
  },
  {
    path: ['automations', 'run'],
    summary: 'Run an Orca automation now',
    usage: 'orcaw automations run <id> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'id'],
    positionalArgs: ['id'],
    examples: ['orcaw automations run 2f9e...', 'orcaw automations run --id 2f9e... --json']
  },
  {
    path: ['automations', 'runs'],
    summary: 'List automation run history',
    usage: 'orcaw automations runs [--id <automation-id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'id'],
    examples: ['orcaw automations runs', 'orcaw automations runs --id 2f9e... --json']
  }
]
